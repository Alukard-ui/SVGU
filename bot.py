...
import settings
from models import UserState,Pairs
import handlers
...
class Bot:
    ...
    def on_event(self,event):
        ...
        if random.randint(0,100) < 5:
            self.send_message(settings.RANDOM_ANSWER[random.randint(0,len(settings.RANDOM_ANSWER))],user_id)
            ...
            for intent in settings.INTENTS:
                if any(token in text.lower() for token in intent['tokens']):
                    ...
                    if intent['group_handler']:
                        ...
                        if text_to_send:
                            self.send_message(text= text_to_send,user_id=user_id)
